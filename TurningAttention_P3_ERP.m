...
for i2 = allPartici % Participants
    for i3 = chanToPlot(1,:) % Channels 1:2, 3, or 4
        for i4 = 1:10 % Stimulus Order
            if size(channelNames,1) > 1
                contraChan = chanToPlot(2,chanToPlot(1,:) == i3);
            end
            % Ipsi Channel
            d{1}{i4} = [d{1}{i4}; ...
                UNFOLD{i2}{i3}{i4}.erp_20_contra_right;UNFOLD{i2}{i3}{i4}.erp_20_contra_left];
            d{2}{i4} = [d{2}{i4}; ...
                UNFOLD{i2}{i3}{i4}.erp_20_ipsi_right;UNFOLD{i2}{i3}{i4}.erp_20_ipsi_left];
            d{3}{i4} = [d{3}{i4}; ...
                UNFOLD{i2}{i3}{i4}.erp_45_contra_right;UNFOLD{i2}{i3}{i4}.erp_45_contra_left];
            d{4}{i4} = [d{4}{i4}; ...
                UNFOLD{i2}{i3}{i4}.erp_45_ipsi_right;UNFOLD{i2}{i3}{i4}.erp_45_ipsi_left];
            d{5}{i4} = [d{5}{i4}; ...
                UNFOLD{i2}{i3}{i4}.erp_90_contra_right;UNFOLD{i2}{i3}{i4}.erp_90_contra_left];
            d{6}{i4} = [d{6}{i4}; ...
                UNFOLD{i2}{i3}{i4}.erp_90_ipsi_right;UNFOLD{i2}{i3}{i4}.erp_90_ipsi_left];

            chanName = channelNames(1,:);
        end
        for i4 = 1:6
            for i5 = 1:10
                idx = size(d{i4}{i5},1)-1:size(d{i4}{i5},1);
                d{i4}{i5}(idx(1),:) = mean(d{i4}{i5}(idx,:));
                d{i4}{i5}(idx(2:end),:) = [];
            end
        end
    end
...
for i2 = 1:6
    for i1 = 1:10
        d{i2}{i1} = squeeze(mean(reshape(d{i2}{i1},length(allPartici),size(chanToPlot,2),200),2));
    end
end
temp = cell(2,6);
for i = 1:6
...
% clear channelNames
exportGraphics  = 0;
exportTxt       = '_P3_StimuliOrder_Based';
thres           = 15;
lineWidth       = 1.5;
permTest        = 5000;
peakDirection   = 'positive';       % P1 -> 'positive' // N2pc -> 'mean'
timeWindow      = [200 400];        % For the P1 in O1/2 -> [100 200] // N2pc in PO7/8 -> [230 430]
erpYlim         = [-1 20];
dCorrYlim       = [2 12];
rtYlim          = [.25 .5];
meanERP         = cell(1,2);
...
counter = 1;
title(t,['Cz, P3 in ' num2str(timeWindow(1)) '-' num2str(timeWindow(2)) 'ms']);
...
% whichWalls      = [1:3 8:10];
exportGraphics  = 0;
exportTxt1      = '_Final_P3a_DifficultyXLaterality';
exportTxt2      = '_Final_P3a_TurningAngleXDifficultyXLaterality';
chanTitle       = 'Pz + Cz + FCz';
erpTitle        = 'P3';
minY            = -5;               % P1 -> -4 // N2pc -> -3
maxY            = 5;                % P1 -> 1.5 // N2pc -> 2
statsYPos       = [12 1.5];          % P1 -> [4 1] // N2pc -> [5 1.2]
RCminmax        = [0 17];          % P1 -> [-3 7] // N2pc -> [-10 8]
statTest        = 'wilcoxon';
...
%------------------------------------
f = figure; f.Color = [1 1 1]; f.Position = [100 25 800 200];
t = tiledlayout(1,9,'TileSpacing','compact','Padding','none');
...
for i1 = 1:3
    ...
    for i = condLoop(i1,:)
        ...
        p1 = plot(UNFOLD{end}, mean(dd{2,i},'omitnan'),'color',mycmap(i,:),'LineWidth',lineWidth,'LineStyle',':');
        yticks(ax1,-6:2:6);
    end
    ...
    [~,stats] = bbar_raincloud(temp,'rmOutlier',rmOutlier,'showMean',1,'stats',statTest,'comparison',[1 2; 3 4; 1 3; 2 4],'nexttile',i1*3,'showMedian',1,...
        ...
        'ylabel', 'µV','StatsYPos', statsYPos,'ylim',RCminmax, 'normal', showNormal,'multcmp','none');
    allStats = [allStats; stats];
end


ff = figure; ff.Color = [1 1 1]; ff.Position = [500 25 800 320];
tiledlayout(2,6,'TileSpacing','compact','Padding','none'); tileCounter = -5; tileCounterRC = 0;
for i2 = 1:2
    tileCounter = tileCounter + 6; 
...
% Parameters
%------------------------------------
timeToAnalyze   = [260 270];        % P1 --> [150 200] 
colorMinMax     = [-3 3];       % P1 --> [-2 2]
exportGraphics  = 1;
exportTxt       = '_Final_Topo_P3a';
exportTxt2      = '_Final_Topo_P3a_Ungrouped';
...
% figure; imagesc(1:100); colormap(mycmap)
...
%------------------------------------
f = figure;
f.Position = [100 100 900 320];
tt = tiledlayout(2,6,'TileSpacing','tight','Padding','tight');
...
for i = 1:6
    for i2 = 1:2
        counter = counter + 1;
        nexttile(counter)
        topoplot(mean(topoD{i2,i}(:,t(1):t(2)),2,'omitnan'), EEG.chanlocs, 'numcontour', 5,'electrodes', 'off','style','fill');
