 ...
 public class TrainingConsoleToGUI : MonoBehaviour
 {
   ...
   string waitingData;
   private float anchorX = 570;
   private float anchorY = 190f;
   /*
   ...
   */
   ...
   void Update() { if (Input.GetKeyDown(KeyCode.Space)) { doShow = !doShow; } }
     ...
     void OnGUI()
     {
         ...
         if (GUI.Button(new Rect(anchorX, anchorY +160, 220, 30), "Reset this trial")){
            m.TrainingDone=false;
            m.breakTime=false;
            m.isDark=false;
            m.hasAnswered=true;
            //m.CurrentTrial += 1;
