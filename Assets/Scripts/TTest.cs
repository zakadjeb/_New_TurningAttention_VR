...
public class TTest : MonoBehaviour
{
    ...
    void Start()
    {
        p = GameObject.Find("[CameraRig]").GetComponent<PosnerParadigm>();
    ...
    void Update()
    {
        ...
        if (runOnce && (p.showRightStimulus || p.showLeftStimulus)) { timer.Reset(); timer.Start(); runOnce = false;}
        if (p.hasAnswered && !runOnce) {
            timer.Stop();
